...
def _write_figures(rows: list[dict], station: dict, target: Path, result: dict) -> None:
    ...
    test = [row for row in rows if result["evaluation_period"]["start"] <= row["date"] <= result["evaluation_period"]["end"]]
    ...
    map_body = (
        ...
        '<text x="310" y="180" font-family="system-ui" font-size="12" fill="#7c3aed">SMAP/GLDAS footprint: unavailable in v0.2</text>'
...
def _final_answer(result: dict) -> str:
    ...
    coverage_status = "meets" if kalman["coverage_95"] >= 0.90 else "fails"
    return (
        f"The strongest baseline is {best} (RMSE {best_rmse:.4f} m³/m³). "
        f"Kalman RMSE is {kalman['rmse']:.4f} m³/m³ versus ERA5 {era5['rmse']:.4f} "
        f"({change:.1%} relative improvement); 95% coverage is {kalman['coverage_95']:.2%} and {coverage_status} the 90% task gate. "
        "The conclusion is restricted to one station; SMAP/GLDAS products and footprints are unavailable in v0.2."
    )


def render_task_report(task: dict, result: dict, trajectory: dict, rows: list[dict], station: dict, run_dir: Path) -> Path:
    _write_figures(rows, station, run_dir / "figures", result)
    persistence_rmse = result["metrics"].get("persistence", {}).get("rmse")
    metric_rows = "".join(
        f"<tr><td>{rank}</td><td>{html.escape(name)}</td><td>{value['n']}</td><td>{value['rmse']:.4f}</td><td>{value['bias']:.4f}</td>"
        f"<td>{value['ubrmse']:.4f}</td><td>{value['correlation']:.3f}</td><td>{value['nse']:.3f}</td><td>{value['kge']:.3f}</td></tr>"
        for rank, (name, value) in enumerate(sorted(result["metrics"].items(), key=lambda item: item[1]["rmse"]), start=1)
    )
    ranking_rows = "".join(
        f"<tr><td>{rank}</td><td>{html.escape(name)}</td><td>{value['rmse']:.4f}</td>"
        f"<td>{'yes' if persistence_rmse is not None and value['rmse'] < persistence_rmse else ('baseline' if name == 'persistence' else 'N/A')}</td></tr>"
        for rank, (name, value) in enumerate(sorted(result["metrics"].items(), key=lambda item: item[1]["rmse"]), start=1)
    )
    step_cards = "".join(
    ...
    leakage = trajectory["evaluation"]["leakage_audit"]
    access_policy = html.escape(json.dumps({"allowed_inputs": task["allowed_inputs"], "forbidden_inputs": task["forbidden_inputs"]}, indent=2))
...
<div class="grid"><section class="panel"><h2>Task mode and data policy</h2><p><b>Mode:</b> {html.escape(task['mode'])}</p><pre>{access_policy}</pre><h2>Data availability</h2><table><tr><th>ISMN labels</th><th>ERA5</th><th>SMAP</th><th>GLDAS</th></tr><tr><td>hidden evaluator truth</td><td>available</td><td>unavailable</td><td>unavailable</td></tr></table><h2>Hydrologic evidence</h2><div class="figures"><img src="figures/map.svg"><img src="figures/scatter.svg"><img src="figures/time_series.svg"><img src="figures/uncertainty.svg"></div><h2>Held-out metrics</h2><table><tr><th>Rank</th><th>Method</th><th>N</th><th>RMSE</th><th>Bias</th><th>ubRMSE</th><th>R</th><th>NSE</th><th>KGE</th></tr>{metric_rows}</table><h2>Baseline ranking</h2><table><tr><th>Rank</th><th>Method</th><th>RMSE</th><th>Beats persistence</th></tr>{ranking_rows}</table></section>
<aside class="panel"><h2>Tool trajectory</h2>{step_cards}</aside></div>
<div class="grid"><section class="panel"><h2>Ground truth vs agent path</h2><b>Ground truth</b><p class="path">{html.escape(gold)}</p><b>Agent</b><p class="path">{html.escape(agent)}</p><h2>Leakage / allowed-input audit</h2><p><b>Status:</b> {'PASS' if leakage['passed'] else 'FAIL'} · violations={leakage['violations']}</p><pre>{html.escape(json.dumps(leakage['step_audits'], indent=2, ensure_ascii=False))}</pre><h2>Scientific boundary</h2><ul>{limitations}</ul></section><section class="panel"><h2>Trajectory scores</h2><div class="e2e">{trajectory['evaluation']['end_to_end']:.3f}</div><p>End-to-end weighted score</p><table>{score_rows}</table><h2>Execution environment</h2><pre>{html.escape(json.dumps(result['environment'], indent=2, ensure_ascii=False))}</pre></section></div></main></html>"""
...
def render_benchmark_summary(results: list[dict], target: Path) -> Path:
    row_parts = []
    for item in results:
        suite_link = target.parent / "tasks" / item["task_id"] / "report.html"
        link = f"tasks/{item['task_id']}/report.html" if suite_link.exists() else "report.html"
        row_parts.append(
            f"<tr><td><a href='{link}'>{html.escape(item['task_id'])}</a></td><td>{html.escape(item['mode'])}</td>"
            f"<td>{html.escape(item['best_method'])}</td><td>{item['metrics']['kalman_analysis']['rmse']:.4f}</td>"
            f"<td>{item['metrics']['kalman_analysis']['coverage_95']:.2%}</td><td>{item['trajectory_score']['end_to_end']:.3f}</td></tr>"
        )
    rows = "".join(row_parts)
    target.write_text(
        f"<!doctype html><meta charset='utf-8'><title>Hydro-Bench Summary</title><style>body{{font:15px system-ui;max-width:1100px;margin:40px auto}}table{{border-collapse:collapse;width:100%}}td,th{{padding:10px;border-bottom:1px solid #ddd;text-align:left}}</style><h1>Hydro-Bench v0.2 Summary</h1><table><tr><th>Task</th><th>Mode</th><th>Best method</th><th>Kalman RMSE</th><th>Coverage 95%</th><th>Trajectory E2E</th></tr>{rows}</table>",
        encoding="utf-8",
    )
    return target
